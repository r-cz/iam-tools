...
const MAX_RETRIES = timeoutArg ? parseInt(timeoutArg.split('=')[1], 10) : DEFAULT_TIMEOUT
...
const checkServer = (port, serverName, retries = 0) => {
  if (interactive) {
    log(
      `Checking if ${serverName} is running on port ${port} (attempt ${retries + 1}/${MAX_RETRIES})...`
    )
  }
