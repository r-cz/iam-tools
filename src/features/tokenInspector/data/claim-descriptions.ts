...
const claimDescriptions: ClaimDescription[] = [
  ...
  },
  {
    name: 'nonce',
    description:
      'String value used to associate a client session with an ID Token (mitigates replay attacks)',
    specification: 'OIDC Core',
    required: false,
    tokenTypes: ['id_token'],
  ...
  // OAuth 2.0 Access Token claims
  {
    ...
    example:
      '[{"type":"account_information","actions":["list_accounts","read_transactions"],"locations":["https://example.com/accounts"]},{"type":"payment_initiation","actions":["initiate","cancel"],"locations":["https://example.com/payments"],"instructedAmount":{"currency":"EUR","amount":"123.50"}}]',
  },
  {
    name: 'scope',
