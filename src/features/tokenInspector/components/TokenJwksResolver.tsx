...
export function TokenJwksResolver({ 
  ...
  // Track when issuerUrl changes from external sources
  // This is to detect auto-population from token payload
  useEffect(() => {
    // If issuerUrl is set and we haven't marked as auto-populated yet
    if (issuerUrl && !isAutoPopulated) {
      setIsAutoPopulated(true);
      
      // Auto-fetch JWKS if we detect a new issuer URL from a token
      // We'll add a small delay to ensure the UI updates first
      const timer = setTimeout(() => {
        console.log('Auto-fetching JWKS for newly detected issuer:', issuerUrl);
        fetchJwks();
      }, 500);
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  // When user manually changes the URL, it's no longer auto-populated
              ...
              {issuerUrl && isAutoPopulated && (
                <div className="text-xs text-green-600 mt-1 flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-3 h-3 mr-1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"></path><path d="M12 5v14"></path></svg>
                  Auto-populated from token's "iss" claim
                </div>
              )}
            </div>
            <Button 
