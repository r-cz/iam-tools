    ...
    <div className="space-y-4">
      ...
      {signatureValid !== undefined && ( // Only show if validation has been attempted (valid could be true or false)
        ...
        >
          ...
          <AlertTitle>
            {signatureValid ? 'Signature Valid' : 'Signature Invalid / Not Verified'}
          </AlertTitle>
          <AlertDescription>
            {signatureValid
              ? 'The token signature has been successfully verified against the loaded JWKS.'
              : signatureError ||
                'The token signature could not be verified. Ensure the correct JWKS are loaded.'}
          </AlertDescription>
        </Alert>
      )}
      {signatureValid === undefined &&
        !jwks && ( // Show info if validation hasn't run because JWKS aren't loaded
          <Alert className="bg-blue-500/10 border-blue-500/20 text-blue-700">
            <Info className="h-4 w-4" />
            <AlertTitle>JWKS Needed for Validation</AlertTitle>
            <AlertDescription>
              Load the JSON Web Key Set (JWKS) using the options below to verify the token's
              signature.
            </AlertDescription>
          </Alert>
        )}
      ...
      <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
        <div className="px-4 py-3 border-b">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Load the public keys needed to verify the signature.
          </p>
        </div>
        <div className="p-4">
          <TokenJwksResolver
          ...
          <div className="pt-3 first:pt-0">
            ...
            {signaturePart ? (
              <CodeBlock
                code={signaturePart}
                language="text"
                className="p-2 text-xs max-h-24 overflow-auto"
              />
            ) : (
              <p className="text-sm italic text-muted-foreground">
                No signature part found in token.
              </p>
            )}
          </div>
        </div>
            ...
            <div className="border-t pt-3">
              ...
              {header?.kid ? (
                ...
                <Alert className="bg-amber-500/10 border-amber-500/20 text-amber-700">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Token header does not contain a Key ID (kid). Validation might require manual
                    key selection if multiple keys exist in JWKS.
                  </AlertDescription>
                </Alert>
              )}
            </div>
