...
export function TokenPayload({ payload, tokenType, validationResults }: TokenPayloadProps) {
  ...
  // Categorize claims
  const commonClaims = Object.keys(payload).filter((key) => standardClaims.includes(key))
  ...
  const formatClaimValue = (key: string, value: any) => {
    ...
    if (['exp', 'iat', 'auth_time', 'nbf'].includes(key) && typeof value === 'number') {
      return (
        <div>
  ...
  const renderClaim = (key: string, value: any) => {
    const relevantResults = validationResults.filter((result) => result.claim === key)
      ...
      >
        <div className="p-4 space-y-3">
          <div className="flex items-start justify-between">
            ...
            <div className="text-sm">{formatClaimValue(key, value)}</div>
          </div>
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            {claimDescription && (
              <>
                ...
                <p className="opacity-80">
                  ...
                  {claimDescription.required &&
                    tokenType === 'id_token' &&
                    ' (Required for ID tokens)'}
                  {claimDescription.required &&
                    tokenType === 'access_token' &&
                    claimDescription.specification.includes('RFC9068') &&
                    ' (Required for RFC9068 JWT access tokens)'}
                </p>
                {claimDescription.specification.includes('RFC9068') &&
                  tokenType === 'access_token' && (
                    <p className="mt-1 text-xs text-blue-600">
                      This claim is defined in the JWT Profile for OAuth 2.0 Access Tokens (RFC9068)
                    </p>
                  )}
              </>
            )}
            ...
            {providerSpecificInfo && (
              <Alert className="mt-2 bg-blue-500/10 border-blue-500/20 text-blue-700">
                ...
                <AlertDescription>
                  {providerSpecificInfo.provider} - {providerSpecificInfo.description}
                </AlertDescription>
              </Alert>
            )}
            ...
            {key === 'aud' && Array.isArray(value) && value.length > 1 && (
              <Alert className="mt-2 bg-amber-500/10 border-amber-500/20 text-amber-700">
                ...
                <AlertDescription>
                  The "azp" claim should be present when there are multiple audiences.
                </AlertDescription>
              </Alert>
            )}
  ...
  return (
    <div className="space-y-4">
      <JsonDisplay data={payload} containerClassName="relative" />

      <div className="space-y-3">
        <h3 className="text-md font-medium">
          {tokenType === 'id_token' ? 'Common OIDC Claims' : 'Common OAuth/JWT Claims'}
        </h3>
      ...
      {otherClaims.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setShowAll(!showAll)} variant="ghost" size="sm">
