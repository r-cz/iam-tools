      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Detailed view of the selected key with ID:{' '}
              <code className="bg-muted px-1">{key.kid}</code>
            </p>
          </div>
              ...
              {key.kty === 'RSA' && (
                <div className="flex gap-2 items-start">
                  ...
                  <p className="text-sm text-amber-700">
                    This is an RSA key. For security reasons, only the public parts (modulus and
                    exponent) are included.
                  </p>
                </div>
              )}
        ...
        <div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>{Object.entries(key).map(([k, v]) => renderKeyParameter(k, v))}</tbody>
          </table>
        </div>
      </div>
    )
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Tabs
          value={view}
          onValueChange={(v) => setView(v as 'formatted' | 'raw')}
          className="w-full"
        >
                    ...
                    {jwks.keys.map((key) => (
                      ...
                      >
                        ...
                        <TableCell>
                          {key.use ? (
                            <Badge
                              variant={key.use === 'sig' ? 'default' : 'secondary'}
          ...
          <TabsContent value="raw">
            <JsonDisplay data={jwks} containerClassName="relative" maxHeight="70vh" />
          </TabsContent>
        </Tabs>
      </CardContent>
      ...
      <CardFooter className="border-t pt-6">
        <div className="text-xs text-muted-foreground">
          <p>
            A JSON Web Key Set (JWKS) contains the public keys that can be used to verify tokens
            issued by the identity provider.
          </p>
          <p className="mt-1">
            These keys are used for signature validation and encryption/decryption operations in
